...
public sealed class CurrentUserServiceTests
{
    ...
    [Fact]
    public void TenantId_WithTenantClaim_ReturnsTenantClaimValue()
    {
        var identity = new ClaimsIdentity(
            new[]
            {
        ...
        service.TenantId.ShouldBe("tenant-42");
    }

    [Fact]
    public void TenantId_WithWorkspaceClaim_FallsBackToWorkspaceId()
    {
        var identity = new ClaimsIdentity(
            new[]
            {
                new Claim(ClaimTypes.NameIdentifier, "user-1"),
                new Claim("workspace_id", "workspace-9"),
            },
            "TestAuth");

        var service = CreateService(new ClaimsPrincipal(identity));

        service.TenantId.ShouldBe("workspace-9");
    }
}
