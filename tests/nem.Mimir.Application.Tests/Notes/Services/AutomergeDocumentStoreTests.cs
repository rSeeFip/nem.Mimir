...
public sealed class AutomergeDocumentStoreTests
{
    ...
    [Fact]
    public async Task ApplyChanges_WithArbitraryBytes_PersistsMergedState()
    {
        var session = Substitute.For<Marten.IDocumentSession>();
        ...
        var store = new AutomergeDocumentStore(session, TimeProvider.System);
        var changes = new byte[] { 0xFE, 0xED, 0xFA, 0xCE };

        var merged = await store.ApplyChanges("doc-legacy", changes, TestContext.Current.CancellationToken);

        merged.Length.ShouldBeGreaterThan(0);
        (await store.LoadState("doc-legacy", TestContext.Current.CancellationToken)).SequenceEqual(merged).ShouldBeTrue();
    }
}
