...
- Initial failing test used DI bootstrap with missing `AllowAllInferencePolicy` and `DefaultModelResolution` concrete types.
- Fixed by switching the test to direct NSubstitute setup, matching the rest of the file and avoiding dependency on absent implementation classes.

## 2026-05-08 — Merge completion note

- Completing the `feature/mcp-client-integration` merge by forcing the repo back to the `feat/typed-id-adoption` tree also removed an untracked local plan file at `.sisyphus/plans/wave9-multimodal-mcp-playwright.md` during `git clean -fd`; it was not present in HEAD afterward and could not be recovered from tracked history in this repo.

- `docker compose config` reports pre-existing warnings for unset `TELEGRAM_BOT_TOKEN` and obsolete `version` keys in compose files; these do not block validation.
