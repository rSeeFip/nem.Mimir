...
public static class DependencyInjection
{
        ...
        IConfiguration configuration)
    {
        ...
        // LLM service
        services.AddScoped<LiteLlmClient>();
        services.AddScoped<ILlmService>(sp =>
        {
            var inner = sp.GetRequiredService<LiteLlmClient>();
            var guardrailed = new GuardrailLlmServiceDecorator(
                inner,
                sp.GetRequiredService<IGuardrailEvaluator>(),
                ResolveActiveGuardrailBundle(sp));
            var budgeted = new BudgetLlmServiceDecorator(
                guardrailed,
                sp.GetRequiredService<ITokenBudgetPolicy>(),
                sp.GetService<ICurrentUserService>(),
                sp.GetService<IHttpContextAccessor>(),
                sp.GetService<ILogger<BudgetLlmServiceDecorator>>());
            var overlay = sp.GetRequiredService<PolicyIntentOverlayService>();
            return new PolicyLlmServiceDecorator(budgeted, overlay);
        });
        services.AddScoped<IInferencePolicy, AllowAllInferencePolicy>();
        services.AddScoped<IModelResolution, DefaultModelResolution>();
        services.AddScoped<PolicyIntentOverlayService>();
        services.AddScoped<IPolicyIntentOverlay>(sp => sp.GetRequiredService<PolicyIntentOverlayService>());
    ...
    private static IGuardrailBundle? ResolveActiveGuardrailBundle(IServiceProvider serviceProvider)
    {
        var options = serviceProvider.GetRequiredService<IOptions<GuardrailsOptions>>().Value;
        if (!options.Enabled || string.IsNullOrWhiteSpace(options.ActiveBundle))
        {
            return null;
        }

        var availableBundles = options.AvailableBundles;
        if (availableBundles.Count > 0
            && !availableBundles.Any(bundle => string.Equals(bundle, options.ActiveBundle, StringComparison.OrdinalIgnoreCase)))
        {
            return null;
        }

        return serviceProvider
            .GetServices<IGuardrailBundle>()
            .FirstOrDefault(bundle => string.Equals(bundle.Name, options.ActiveBundle, StringComparison.OrdinalIgnoreCase));
    }
}
