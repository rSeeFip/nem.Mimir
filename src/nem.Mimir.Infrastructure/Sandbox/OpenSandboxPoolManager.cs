...
{
    private readonly ISandboxContract _sandboxContract;
    private readonly ConcurrentBag<ISandboxSession> _pool = [];
    private int _totalCreated;
    private int _totalDestroyed;

    public OpenSandboxPoolManager(ISandboxContract sandboxContract)
    {
        ArgumentNullException.ThrowIfNull(sandboxContract);
        _sandboxContract = sandboxContract;
    }
    ...
    public async Task WarmPoolAsync(int count, SandboxConfig config, SandboxPolicyClass policyClass = SandboxPolicyClass.CreationLocked, CancellationToken cancellationToken = default)
    {
        ...
        {
            var session = await _sandboxContract.AcquireAsync(config, cancellationToken).ConfigureAwait(false);
            _pool.Add(session);
    ...
    public async Task DrainPoolAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            await _sandboxContract.ReleaseAsync(new SandboxSessionId(session.SessionId), cancellationToken).ConfigureAwait(false);
