...
try
{
    ...
    builder.Services.AddNemObservability(builder.Configuration);
    ...
    builder.UseWolverine(opts =>
    {
        ...
        opts.Discovery.IncludeAssembly(typeof(MimirFederationPeerHealthUpdateHandler).Assembly);
        opts.Discovery.IncludeAssembly(typeof(ReplayDeadLetterHandler).Assembly);
    ...
    builder.Services.AddNemVersionedMessageInfrastructure();

    // ── API Services ─────────────────────────────────────────────────────────
    builder.Services.AddHttpContextAccessor();
    builder.Services.AddScoped<ICurrentUserService, CurrentUserService>();
    builder.Services.AddSingleton(TimeProvider.System);
    builder.Services.Configure<ReplayBufferOptions>(builder.Configuration.GetSection(ReplayBufferOptions.SectionName));
    ...
    app.MapHealthChecks("/ready", healthCheckOptions).AllowAnonymous();
