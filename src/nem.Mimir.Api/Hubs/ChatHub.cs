...
public sealed class ChatHub : Hub
{
    ...
    private readonly IReplayBuffer _replayBuffer;
    ...
    public override async Task OnConnectedAsync()
    {
        ...
        {
            await ReplayBufferedMessagesIfNeededAsync(userGuid, Context.ConnectionAborted);
        }
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        var userMessage = conversation.AddMessage(MessageRole.User, content);
        await _repository.UpdateAsync(conversation, cancellationToken);
        await _unitOfWork.SaveChangesAsync(cancellationToken);
        await _replayBuffer.RecordMessageAsync(
            conversation.Id,
            userGuid,
            new ReplayMessage(userMessage.Role, userMessage.Content, userMessage.CreatedAt),
            cancellationToken);
        ...
        _ = ProduceTokensAsync(channel.Writer, conversation, userGuid, selectedModel, llmMessages, ragSources, responseBuilder, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (fullResponse.Length > 0)
            {
                ...
                await _replayBuffer.RecordMessageAsync(
                    conversation.Id,
                    userId,
                    new ReplayMessage(assistantMessage.Role, finalResponse, assistantMessage.CreatedAt),
        ...
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        {
            ...
            _logger.LogWarning("Client disconnected during streaming for conversation {ConversationId}",
                conversation.Id);
            await PersistPartialResponse(conversation, userId, responseBuilder, model);
        }
        catch (Exception ex)
        {
        ...
        try
        {
            ...
            if (partialResponse.Length > 0)
            {
                ...
                await _replayBuffer.RecordMessageAsync(
                    conversation.Id,
                    userId,
                    new ReplayMessage(assistantMessage.Role, assistantMessage.Content, assistantMessage.CreatedAt),
                    CancellationToken.None);
    ...
    private async Task ReplayBufferedMessagesIfNeededAsync(Guid userId, CancellationToken cancellationToken)
    {
        ...
        var replayMessages = await _replayBuffer.GetReplayAsync(conversationGuid, userId, cancellationToken);
        ...
        _logger.LogInformation(
            "Replaying {ReplayCount} buffered messages for conversation {ConversationId} to connection {ConnectionId}",
            replayMessages.Count,
            conversationGuid,
            Context.ConnectionId);

        await Clients.Caller.SendAsync("ReplayMessages", replayMessages, cancellationToken);
    }
